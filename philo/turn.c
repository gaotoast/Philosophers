...
/*   Updated: 2025/08/06 15:18:09 by stakada          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	is_my_turn(int philo_id, t_data *data)
{
	...
	if (philo_id % 2 != 0 && turn == TURN_ODD)
		return (1);
	if (philo_id % 2 == 0 && turn == TURN_EVEN)
		return (1);
	if (philo_id == data->n_of_philos && turn == TURN_LAST)
		return (1);
	return (0);
}
...
static void	update_current_turn(t_data *data)
{
	...
	half_philos = data->n_of_philos / 2;
	is_philos_odd = data->n_of_philos % 2;
	if (data->current_turn == TURN_ODD && data->odd_done == half_philos)
	{
		data->odd_done = 0;
		data->current_turn = TURN_EVEN;
	}
	else if (data->current_turn == TURN_EVEN && data->even_done == half_philos)
	{
		data->even_done = 0;
		if (is_philos_odd)
